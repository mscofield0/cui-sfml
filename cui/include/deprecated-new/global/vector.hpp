#ifndef CUI_GLOBAL_VECTOR_HPP
#define CUI_GLOBAL_VECTOR_HPP
...
public:
	...
	Vector(Args&&... args) : vec_{std::forward<Args>(std::move(args))...} {}
	...
	auto insert(const_iterator pos, value_type&& value) {
		return vec_.insert(pos, std::move(value));
	}
	...
	auto emplace(const_iterator pos, Args&&... args) {
		return vec_.emplace(pos, std::move(args)...);
	}
	...
	auto push_back(value_type&& value) {
		return vec_.push_back(std::move(value));
	}

	template <typename... Args>
	auto emplace_back(Args&&... args) {
		return vec_.emplace_back(std::move(args)...);
	}
